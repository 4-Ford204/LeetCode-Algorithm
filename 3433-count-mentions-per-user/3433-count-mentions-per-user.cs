public class Solution {
    public int[] CountMentions(int numberOfUsers, IList<IList<string>> events) {
        var sortedEvents = events.OrderBy(e => int.Parse(e[1])).ThenBy(e => e[0] == "MESSAGE" ? 1 : 0).ToList();
        int[] count = new int[numberOfUsers];
        int[] nextOnlineTime = new int[numberOfUsers];

        foreach (var eventItem in sortedEvents) {
            int currentTime = int.Parse(eventItem[1]);
            ...
            if (type == "MESSAGE") {
                ...
                if (target == "ALL") {
                    ...
                    for (int i = 0; i < numberOfUsers; i++) {
                        if (nextOnlineTime[i] <= currentTime) count[i]++;
                    }
                }
                else {
                ...
                nextOnlineTime[idx] = currentTime + 60;
            }
        }
